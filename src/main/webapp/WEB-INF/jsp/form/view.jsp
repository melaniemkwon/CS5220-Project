...
<div class="container">
    ...
    <c:if test="${empty items}">
        <h4>This form has no questions yet.</h4>
    </c:if>
    ...
    <c:if test="${not empty items}">
            ...
            <c:forEach items="${items}" var="item">
                <tr>
                    ...
                    <td>
	                    ...
	                    <input type="text" name="textItem"><br>
	                    </c:if>
	                    ...
	                    <c:if test="${item.itemType == 'CHECKBOX' || item.itemType == 'DROPDOWN_LIST' || item.itemType == 'MULTIPLE_CHOICE' }">
		                  <c:forEach items="${item.selections}" var="selection">
		                      <input type="checkbox" name="${selection.id}" value="${selection.value}"> ${selection.value}<br>
		                  </c:forEach>
	                    </c:if>     
	                    <br>
	                    
	                    <div style="color:red;">
						<c:if test="${item.required}">*required</c:if>
						</div>                   
                    </td>
                    </c:if>
                </tr>                   
        ...
        <a href="#"><button type="button" class="btn btn-default">Save Progress</button></a>
        <input type="submit" class="btn btn-info" name="add" value="Submit Form" />
        </form>
    </c:if>
